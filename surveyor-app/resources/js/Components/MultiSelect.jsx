        ...
        <div className={className}>
            {label && (
                <label className={`block text-sm font-medium mb-2 ${isDark ? 'text-emerald-100' : 'text-gray-700'}`}>
                    ...
                    {maxItems && (
                        <span className={`ml-2 text-xs font-normal ${isDark ? 'text-emerald-500/50' : 'text-gray-400'}`}>
                            (maks. {maxItems} area)
                        </span>
                    )}
                </label>
            ...
            <div ref={containerRef} className="relative">
                ...
                <button
                    ...
                    disabled={disabled}
                    className={`w-full min-h-[42px] flex items-center justify-between gap-2 px-3 py-2 text-sm rounded-lg transition-colors text-left
                        ${disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
                        ${error
                            ? 'border-red-500 focus:ring-red-500'
                            : isDark 
                                ? 'bg-emerald-950/30 border border-emerald-900/50 hover:border-emerald-500/50 focus:ring-emerald-500/50' 
                                : 'bg-white border border-gray-300 hover:border-teal-400 focus:ring-teal-500'
                        }
                ...
                >
                    <div className="flex-1 flex flex-wrap gap-1.5">
                        {selectedOptions.length === 0 ? (
                            <span className={isDark ? 'text-emerald-500/40' : 'text-gray-400'}>
                                {placeholder}
                            </span>
                        ) : (
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        className={`p-0.5 rounded transition-colors
                                            ${isDark ? 'hover:bg-emerald-500/30' : 'hover:bg-teal-200'}
                                        `}
                                    >
                                        <X className="w-3 h-3" />
                                    </button>
                    ...
                    `}>
                        ...
                        {isMaxReached && (
                            <div className={`px-3 py-2 text-xs border-b
                                ${isDark ? 'bg-amber-500/10 text-amber-400 border-emerald-900/30' : 'bg-amber-50 text-amber-600 border-gray-100'}
                            `}>
                                Maksimal {maxItems} area sudah dipilih
                            </div>
                        )}
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between gap-2 px-3 py-2 text-sm text-left transition-colors
                                                ${isDisabled
                                                    ? isDark ? 'text-emerald-500/30 cursor-not-allowed' : 'text-gray-300 cursor-not-allowed'
                                                    : isSelected
