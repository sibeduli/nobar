...
const getMarkerIcon = (status) => {
    const colors = {
        ...
        pending: '#f59e0b',
        rejected: '#ef4444',
        mixed: '#6366f1', // Purple for multiple venues at same location
    };
    return {
...
// Venue type labels for filtering
const venueTypeLabels = [
    'Komersial Terverifikasi',
    'Non-Komersial', 
    'Pelanggaran QR',
...
export default function VenuesMap({ venues: initialVenues = [] }) {
    ...
    const handleRefresh = () => {
        ...
        router.visit('/venues/map', {
            preserveScroll: true,
            onFinish: () => setIsRefreshing(false),
        });
    };

    // Group venues by location (for handling overlapping markers)
    const getVenuesAtLocation = (lat, lng) => {
        return venues.filter(v => v.lat === lat && v.lng === lng);
    };

    // Get unique locations for markers (to avoid duplicate markers at same spot)
    const uniqueLocations = venues.reduce((acc, venue) => {
        const key = `${venue.lat},${venue.lng}`;
        if (!acc.find(v => `${v.lat},${v.lng}` === key)) {
            acc.push(venue);
        }
        return acc;
    }, []);

    // Get unique venue types and areas from data
    const venueTypes = [...new Set(venues.map(v => v.venueType))].filter(Boolean);
                ...
                <div className={`flex-1 rounded-xl overflow-hidden ${isDark ? 'border border-emerald-900/30' : 'border border-gray-200'}`}>
                    {isLoaded ? (
                        ...
                        >
                            ...
                            {uniqueLocations.filter(venue => {
                                if (filters.status.length > 0 && !filters.status.includes(venue.status)) return false;
                                if (filters.venueType.length > 0 && !filters.venueType.includes(venue.venueType)) return false;
                                if (filters.area.length > 0 && !filters.area.includes(venue.area)) return false;
                                return true;
                            }).map(venue => {
                                const venuesHere = getVenuesAtLocation(venue.lat, venue.lng);
                                const hasMultiple = venuesHere.length > 1;
                                // Use a mixed color marker if multiple venues at same location
                                const markerStatus = hasMultiple ? 'mixed' : venue.status;
                                return (
                                    <Marker
                                        key={`${venue.lat},${venue.lng}`}
                                        position={{ lat: venue.lat, lng: venue.lng }}
                                        icon={getMarkerIcon(markerStatus)}
                                        label={hasMultiple ? { text: String(venuesHere.length), color: '#ffffff', fontSize: '11px', fontWeight: 'bold' } : undefined}
                                        onClick={() => setSelectedLocation({ lat: venue.lat, lng: venue.lng, venues: venuesHere })}
                                    />
                                );
                            })}
                            ...
                            {selectedLocation && (
                                ...
                                >
                                    <div className="min-w-[250px] max-w-[300px] p-1">
                                        {selectedLocation.venues.length > 1 && (
                                            <p className="text-xs text-gray-500 mb-2 pb-2 border-b border-gray-200">
                                                {selectedLocation.venues.length} venue di lokasi ini
                                            </p>
                                        )}
                                        <div className="space-y-3 max-h-[300px] overflow-y-auto">
                                            {selectedLocation.venues.map((venue, idx) => (
                                                <div key={venue.id} className={idx > 0 ? 'pt-3 border-t border-gray-100' : ''}>
                                                    <div className="flex items-start justify-between gap-2 mb-1">
                                                        <h3 className="font-semibold text-gray-900 text-sm">{venue.venueName}</h3>
                                                    </div>
                                                    <span className={`inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full mb-1
                                                        ${venue.status === 'approved' ? 'bg-emerald-100 text-emerald-700' : 
                                                          venue.status === 'pending' ? 'bg-amber-100 text-amber-700' : 
                                                          'bg-red-100 text-red-700'}
                                                    `}>
                                                        {venue.status === 'approved' ? 'Disetujui' : 
                                                         venue.status === 'pending' ? 'Pending' : 'Ditolak'}
                                                    </span>
                                                    <p className="text-xs text-gray-500">{venue.venueType}</p>
                                                    {venue.address && <p className="text-xs text-gray-600 mt-1">{venue.address}</p>}
                                                    <div className="flex items-center gap-1 mt-1">
                                                        <Phone className="w-3 h-3 text-gray-400" />
                                                        <span className="text-xs text-gray-600">{venue.contactPerson} - {venue.phone}</span>
                                                    </div>
                                                    <a
                                                        href={`/surveys?openSurvey=${venue.id}`}
                                                        className="flex items-center justify-center gap-1 mt-2 px-2 py-1 bg-gray-100 text-gray-700 text-xs rounded hover:bg-gray-200 transition-colors"
                                                    >
                                        ...
                                        <a
                                            ...
                                            className="flex items-center justify-center gap-1 mt-3 px-3 py-1.5 bg-teal-600 text-white text-xs rounded-lg hover:bg-teal-700 transition-colors"
                                        >
